...
$where = [];
// Ambil perawatan berjalan: Belum Dimulai + Sedang Proses
$where[] = "status IN ('Belum Dimulai','Sedang Proses')";

if ($search != '') {
    $searchEsc = mysqli_real_escape_string($koneksi, $search);
    $where[] = "(nama_aset LIKE '%$searchEsc%' OR teknisi LIKE '%$searchEsc%')";
}
if ($dari != '') {
    $dariEsc = mysqli_real_escape_string($koneksi, $dari);
    $where[] = "tanggal >= '$dariEsc'";
}
if ($sampai != '') {
    $sampaiEsc = mysqli_real_escape_string($koneksi, $sampai);
    $where[] = "tanggal <= '$sampaiEsc'";
}

$whereSQL = count($where) ? 'WHERE ' . implode(' AND ', $where) : '';

// ==================== QUERY DATA ==================== //
$sql = "SELECT * FROM perawatan $whereSQL ORDER BY tanggal DESC, nama_aset ASC, id DESC";
...
$pdf->Cell(0, 10, 'LAPORAN DATA PERAWATAN BERJALAN', 0, 1, 'C');
$pdf->Ln(4);
...
if (mysqli_num_rows($res) == 0) {
    $pdf->Cell(array_sum($widths), 9, 'Tidak ada data perawatan berjalan', 1, 1, 'C');
} else {
