            ...
            <div class="container-form">
                <div class="card">
                    <div class="card-header">
                        <i class="bi bi-tools"></i> Jadwalkan Perawatan / Kalibrasi Baru
                    </div>
                    <div class="card-body p-4">
                        <form method="post">

                            <div class="mb-4">
                                <label class="form-label fw-bold">Pilih Aset Utama</label>
                                <select name="nama_aset" class="form-select" required>
                                    <option value="">-- Pilih Aset yang Akan Dirawat --</option>
                                    <?php while ($a = mysqli_fetch_assoc($aset_query)) : ?>
                                        <option value="<?= htmlspecialchars($a['nama_aset']) ?>">
                                            <?= htmlspecialchars($a['nama_aset']) ?> — [ Kategori: <?= htmlspecialchars($a['kategori_aset']) ?> | 📍 <?= htmlspecialchars($a['lokasi']) ?> ]
                                        </option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                            ...
                            <div class="row g-2 mt-2">
                                <div class="col-sm-8">
                                    <button type="submit" name="simpan" class="btn btn-success w-100 py-2">
